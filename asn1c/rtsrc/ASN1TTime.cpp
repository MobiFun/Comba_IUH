...
*/
EXTRTMETHOD int ASN1TTime::setYear(int year_) {
...
* Returns 0, if succeed, or error code, if error.
*/
EXTRTMETHOD int ASN1TTime::setMonth(int month_) {
...
* Returns 0, if succeed, or error code, if error.
*/
EXTRTMETHOD int ASN1TTime::setDay(int day_) {
...
* Returns 0, if succeed, or error code, if error.
*/
EXTRTMETHOD int ASN1TTime::setHour(int hour_) {
   if (hour_ < 0 || hour_ > 23)
      return LOG_TTMERR (RTERR_INVFORMAT);
   this->mHour = hour_;
   return 0;
...
* Returns 0, if succeed, or error code, if error.
*/
EXTRTMETHOD int ASN1TTime::setMinute(int minute_) {
...
* Returns 0, if succeed, or error code, if error.
*/
EXTRTMETHOD int ASN1TTime::setSecond(int second_) {
...
*/
EXTRTMETHOD int ASN1TTime::setDiffHour(int dhour) {
...
*          RTERR_INVFORMAT - if value is invalid.
*/
EXTRTMETHOD int ASN1TTime::setDiff(int dhour, int dminute) {
   if (dhour < -12 || dhour > 12 || abs(dminute) > 59)
      return LOG_TTMERR (RTERR_INVFORMAT);
   mDiffHour = dhour;
   if (mDiffHour < 0)
      mDiffMin = -abs(dminute);
   else 
      mDiffMin = abs(dminute);
   return 0;
}
...
*          RTERR_INVFORMAT - if value is invalid.
*/
EXTRTMETHOD int ASN1TTime::setDiff(int inMinutes) {
...
EXTRTMETHOD void ASN1TTime::addDays (int deltaDays, short& year, short& month, short& day)
{

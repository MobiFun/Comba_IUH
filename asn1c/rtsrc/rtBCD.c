...
EXTRTMETHOD int rtStringToBCD (const char* str, OSOCTET* bcdStr, size_t bufsiz,
                   OSBOOL isTBCD)
{
   OSOCTET         elem = 0, sym;
...
EXTRTMETHOD const char* rtBCDToString (OSUINT32 numocts, const OSOCTET* data, 
                           char* buffer, size_t bufsiz, OSBOOL isTBCD)
{
...
/* Returns number of octets are occupied by BCD string in the bcdStr. */
EXTRTMETHOD int rtStringToTBCD (const char* str, OSOCTET* bcdStr,
                                size_t bufsiz)
{
   OSOCTET         elem = 0, sym;
   register size_t i, maxDigits = bufsiz * 2;

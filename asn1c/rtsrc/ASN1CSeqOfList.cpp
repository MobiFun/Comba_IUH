...
EXTRTMETHOD int ASN1CSeqOfListIterator::remove() {
   ...
   OSRTDListNode* next = lastNode->next;
...
EXTRTMETHOD void ASN1CSeqOfList::appendArray(const void* data, int numElems, int elemSize) {
   if (getCtxtPtr() == 0) return; //check the context
...
EXTRTMETHOD void ASN1CSeqOfList::appendArrayCopy(const void* data, int numElems, int elemSize) {
   if (getCtxtPtr () == 0) return; //check the context
...
EXTRTMETHOD void ASN1CSeqOfList::free() {
   if (getCtxtPtr () == 0) return; //check the context
...
EXTRTMETHOD int ASN1CSeqOfList::size() const {
   return pList->count;   
}

// Converts to array
EXTRTMETHOD void* ASN1CSeqOfList::toArray (int elemSize) {
   void* pArray;
   OSUINT32 nelem = 0;
...
// Converts to array
EXTRTMETHOD void* ASN1CSeqOfList::toArray (void* pArray, int elemSize, int allocatedElems) {
   OSUINT32 nelem = allocatedElems;

...
EXTXMLMETHOD int INITCTXTAPPINFOFUNC (OSCTXT* pctxt)
{
   ...
   pctxt->pXMLInfo = rtxMemAllocType (pctxt, OSXMLCtxtInfo);
   if (pctxt->pXMLInfo == 0) return LOG_RTERRNEW (pctxt, RTERR_NOMEM);
   rtxMemHeapMarkSaved (&pctxt->pMemHeap, pctxt->pXMLInfo, TRUE);
...
#ifndef BUILDASN1XERDLL /* to avoid an undefined symbol error */
   rtErrXmlInit ();
#endif
...
EXTXMLMETHOD int FREECTXTAPPINFOFUNC (OSCTXT* pctxt)
{
   if (0 != pctxt->pXMLInfo) {
      ...
      if (0 != pCtxtInfo->pFreeFunc) {
         ...
         rtxMemHeapMarkSaved (&pctxt->pMemHeap, pctxt->pXMLInfo, FALSE);
      }
      pctxt->pXMLInfo = 0;
      return 0;
   }

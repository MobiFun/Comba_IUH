                                ...
                                size_t itemCount)
{
   ...
   nchars = rtxUIntToCharStr (itemCount, numbuf, sizeof(numbuf), 0);
   if (nchars < 0) return LOG_RTERR (pctxt, nchars);

   /* Verify NVP and special chars will fit in encode buffer */

   stat = rtxCheckBuffer (pctxt, nameLen + valueLen + nchars + 5);
   if (stat != 0) return LOG_RTERR (pctxt, stat);

   /* Copy data to buffer */

   OSRTPUTCHAR (pctxt, ' ');
   OSRTMEMCPY (pctxt, name, nameLen);
   OSRTPUTCHAR (pctxt, '=');
   OSRTPUTCHAR (pctxt, '\"');

   OSRTMEMCPY (pctxt, value, valueLen);
   OSRTPUTCHAR (pctxt, '[');
   OSRTMEMCPY (pctxt, numbuf, nchars);
   OSRTPUTCHAR (pctxt, ']');

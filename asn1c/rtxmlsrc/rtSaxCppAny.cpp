...
EXTXMLMETHOD OSXMLAnyHandler::OSXMLAnyHandler 
(OSXMLStringClass& msgData, OSRTContext* pContext, int level):
   OSXMLDefaultHandler (pContext), 
...
EXTXMLMETHOD OSXMLAnyHandler::OSXMLAnyHandler 
(OSXMLStringClass& msgData, OSRTContext* pContext, const OSUTF8CHAR* elemName)
   :   OSXMLDefaultHandler (pContext), 
       mpAnyMsgData (&msgData), mpAnyTypeMsgData (0)
{   
   mLevel = 0;
    ...
    const OSUTF8CHAR* const* attrs)
{
   ...
   if (pEncCtxt->state != OSXMLINIT) {
      ...
      OSRTSAFEPUTCHAR1 (pEncCtxt, '<', stat);
      if (stat != 0) return LOG_SAXERR_AND_SKIP (pctxt, stat);

      OSRTSAFEMEMCPY1 (pEncCtxt, localname, 
                       rtxUTF8LenBytes ((const OSUTF8CHAR*)localname), stat); 
      if (stat != 0) return LOG_SAXERR_AND_SKIP (pctxt, stat);
    ...
    const OSUTF8CHAR* const /*qname*/)
{
   ...
   if (pEncCtxt->state == OSXMLSTART || pEncCtxt->state == OSXMLDATA) {
      ...
      if (mLevel >= 0 && !isEmptyElement (localname)) {
         ...
         OSRTSAFEMEMCPY1 (pEncCtxt, "</", 2, stat);
         if (stat != 0) return LOG_SAXERR_AND_SKIP (pctxt, stat);

         OSRTSAFEMEMCPY1 (pEncCtxt, 
                          localname, 
      ...
      if (isComplete ()) {
         ...
         OSRTSAFEPUTCHAR1 (pEncCtxt, '\0', stat);
         if (stat != 0) return LOG_SAXERR_AND_SKIP (pctxt, stat);
   ...
   if (*bufp == '>') { /* no character data */
      *bufp-- = '\0';  /* null out last '>' character */
      ...
      if (rtXmlStrCmpAsc (qname, bufp)) {
         ...
         OSRTSAFEMEMCPY1 (pEncCtxt, "/>", 2, stat); /* append null element indicator */
         return TRUE;
      }
      else
         OSRTSAFEMEMCPY1 (pEncCtxt, ">", 1, stat);  /* put back original '>' */
   }
   else

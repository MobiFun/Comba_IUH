...
public :
   ...
   virtual unsigned int curPos() const {
      return (unsigned int)source.currentPos ();
   }
...
class OSXercesStringArray {
 ...
 public:
   inline OSXercesStringArray (unsigned sz) {
   ...
   ~OSXercesStringArray ();
   inline void set (unsigned idx, OSXercesString* elem) {
      if (idx >= size) return;
      array [idx] = elem;
...
{ 
   ...
   if (userSaxHandler != 0) {
      ...
      // Convert attributes
      for (unsigned i = 0, n = attrs.getLength (); i < n; i++) {

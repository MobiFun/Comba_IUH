...
EXTXMLMETHOD int rtXmlEncStartAttrC14N (OSCTXT* pctxt)
{
   ...
   OSCRTLMEMCPY (&pCtxtInfo->savedBuffer, &pctxt->buffer, sizeof (OSRTBuffer));
...
EXTXMLMETHOD int rtXmlEncEndAttrC14N (OSCTXT* pctxt)
{
   ...
   OSCRTLMEMCPY (&pctxt->buffer, &pCtxtInfo->savedBuffer, sizeof (OSRTBuffer));
   ...
   while (pNode) {
      ...
      OSRTSAFEMEMCPY (pctxt, attr, pAttrOffs->length + 1);
      pNode = pNode->next;
   }

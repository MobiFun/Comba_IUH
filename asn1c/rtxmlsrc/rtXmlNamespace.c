...
EXTXMLMETHOD const OSUTF8CHAR* rtXmlNSGetPrefix (OSCTXT* pctxt, const OSUTF8CHAR* uri)
{
   ...
   return (0 != plink) ? plink->ns.prefix : OSUTF8("");
}
...
EXTXMLMETHOD OSXMLNamespace* rtXmlNSLookupURI (OSCTXT* pctxt, const OSUTF8CHAR* uri)
{
   ...
   if (0 != plink) {
      if (0 != plink->ns.uri) { 
         ...
         if (rtxUTF8StrEqual 
             (pXMLCtxtInfo->nsURITable.data[plink->nsidx], uri)) {
            plink->ns.uri = pXMLCtxtInfo->nsURITable.data[plink->nsidx];
            return &plink->ns;
         }
      }
   }
 ...
 const OSUTF8CHAR* nsTable[], OSUINT32 nsTableRowCount)
{
   ...
   if (0 == plink) {
      ...
      rtxDynPtrArrayAppend (pctxt, &plink->extraPrefixes, (void*)listPrefix);
   }
   ...
   OSRTDList* pNSAttrs)
{
   ...
   if (0 != pNS) {
      OSXMLNSPfxLink* plink = getPrefixLinkUsingURI (pctxt, pNS->uri);

      if (0 != plink) {
         const OSUTF8CHAR* nsPrefix = 0;

         /* Look for non-null prefix to use for attribute QName */
         if (!OS_ISEMPTY (plink->ns.prefix)) {
            nsPrefix = plink->ns.prefix;
         }
         else if (plink->extraPrefixes.count > 0) {
            int i;
            for (i = 0; i < plink->extraPrefixes.count; i++) {
               nsPrefix = (const OSUTF8CHAR*)plink->extraPrefixes.data[i];
               if (!OS_ISEMPTY (nsPrefix)) {
                  break;
               }
            }
         ...
         if (OS_ISEMPTY (nsPrefix)) {
            nsPrefix = rtXmlNSNewPrefix (pctxt, pNS->uri, pNSAttrs);
         }
         rtxUTF8Strcpy (buf, bufsiz, nsPrefix);
         idx += OSUTF8LEN(nsPrefix);
         buf[idx++] = ':';

...
{ 
   ...
   if (event.mId != OSXMLEVT_START_TAG) {
      rtXmlRdNextEvent (pXmlReader, OSXMLEVT_START_TAG, event.mLevel + 1, 0);
   }
   ...
   */
   if ((nsidx == -1 || nsidx == nsIndex || nsidx == 0) &&
       (localName.length == 0 || 
...
EXTXMLMETHOD void rtXmlpCountListItems (OSCTXT* pctxt, OSUINT32* itemCnt) {
   OSXMLCtxtInfo* xmlCtxt;
   struct OSXMLReader* pReader;
   OSUINT32 cnt = 0;

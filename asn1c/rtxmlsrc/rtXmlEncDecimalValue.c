                          ...
                          char* pDestBuf, size_t destBufSize)
{
   ...
   if (mantissa != 0.0) {
      ...
      if (exp > totalDigits - 1) 
         corrpow = -totalDigits + 1;
      else if (fractionDigits == INT_MAX) {
         corrpow = -PRECISION;
      }
      else if (exp > 0) 
         corrpow = -((fractionDigits + exp < totalDigits) ? 
            fractionDigits + exp : totalDigits - 1);
      else
   ...
   if (firstZeroInFractionPtr != 0 && fractionDigitsNow > fractionMinDigits) {
      ...
      exp += (int)(p - firstZeroInFractionPtr);

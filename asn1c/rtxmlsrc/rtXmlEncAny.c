...
{   
   ...
   if (anyType) {
      ...
      /* memBuf should be freed when done using parseCtxt. */
      rtxMemHeapMarkSaved (&memBuf.pctxt->pMemHeap, memBuf.buffer, FALSE);
   }
   else {
      stat = rtxStreamMemoryCreateReader 
      ...
      if (pXmlReader->mLastEvent.mId == OSXMLEVT_END_DOCUMENT) {
         ...
         if (pXmlReader->mbCDATA && !rtxCtxtTestFlag (pctxt, OSXMLC14N)) {
            if (prevState >= 0)           /* First CDATA chunk. */
               OSRTSAFEMEMCPY (pctxt, "<![CDATA[", 9);

            OSRTSAFEMEMCPY (pctxt, pXmlReader->mData.value, len);

            if (pXmlReader->mState >= 0)  /* Last CDATA chunk. */
               OSRTSAFEMEMCPY (pctxt, "]]>", 3);
         }
         else {         
         ...
         if (hasAttrs) {
            ...
            if (stat > 0) {
               ...
               for (i = 0; i < nAttrs; i++) {
                  ...
                  stat = rtXmlpDecDynUTF8Str (parseCtxt, &value);

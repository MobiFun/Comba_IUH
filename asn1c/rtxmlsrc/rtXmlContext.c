...
EXTXMLMETHOD OSXMLCtxtInfo* rtXmlCtxtAppInfoDup (OSCTXT* pctxt, OSCTXT* pDstCtxt)
{
   ...
   pDstXMLInfo = rtxMemAllocType (pDstCtxt, OSXMLCtxtInfo);
   ...
   rtxDListFastInit (&pDstXMLInfo->namespaceList);
   rtxDListFastInit (&pDstXMLInfo->encodedNSList);
   rtxDListFastInit (&pDstXMLInfo->sortedAttrList);

   pDstXMLInfo->encodingStr = pSrcXMLInfo->encodingStr;

   pDstXMLInfo->pXmlPPReader = pSrcXMLInfo->pXmlPPReader; /* ?? */
...
EXTXMLMETHOD int rtXmlSetEncoding (OSCTXT* pctxt, OSXMLEncoding encoding)
{
   ...
   if (0 != pctxt->pXMLInfo) {
      ...
      if (encoding != OSXMLUTF8) return LOG_RTERRNEW (pctxt, RTERR_NOTSUPP);
...
EXTXMLMETHOD int rtXmlUpdateBOM (OSXMLCtxtInfo* pXMLInfo, OSXMLEncoding encoding)
{
   ...
   case OSXMLUTF8:
      ...
      break;
      /* TODO: add cases for other encodings when support is added */
   }
   return 0;
}

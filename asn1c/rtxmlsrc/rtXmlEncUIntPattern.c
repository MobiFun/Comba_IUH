...
(OSRTMEMBUF* pMemBuf, OSUINT32 value, int* pState,
...
(OSCTXT* pOSCTXT, rtxRegexpPtr regexp, OSUINT32 value, OSUTF8CHAR** ppDest) 
{
   int i, num, j, pos, state = OSRTIPS_EMPTY;
   OSRTMEMBUF membuf;
   ...
   for (i = 0;i < regexp->nbAtoms; i++) {
       ...
       if (regexp->atoms[i]->codepoint != 0) {
           ...
           if (formatRange (&membuf, value, &state, 
                            num, XML_REGEXP_CHARVAL, curCh, curCh, FALSE) < 0)
              return RTERR_REGEXP;
       }
       else {
...
EXTXMLMETHOD int rtXmlEncUIntPatternValue 
(OSCTXT* pctxt, OSUINT32 value, const OSUTF8CHAR* pattern)
{
   int stat = 0;
   ...
   if (stat == 0)
      stat = rtxCopyAsciiText (pctxt, (const char*)pFormatStr);
...
EXTXMLMETHOD int rtXmlEncUIntPattern (OSCTXT* pctxt, OSUINT32 value, 
                        ...
                        const OSUTF8CHAR* pattern)
{
   ...
   if (0 == stat)
      stat = rtXmlEncUIntPatternValue (pctxt, value, pattern);

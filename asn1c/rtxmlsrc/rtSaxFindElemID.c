                  ...
                  OSINT16 fstabRows, OSINT16 fstabCols)
{
   ...
   if (curState >= 0 && curState < fstabCols) {
      OSINT16 i, j = (OSINT16)(prevElemIdx + 1);

      /* start to search from the previous element index + 1 for better 
         performance.
...
#endif
         if (match) {
            const IDXTYPE* pCurRow = fstab + j * fstabCols;
            OSINT16 s;
            
            /* now, search for the first possible state, starting from the
               current one. */
            
            for (s = curState; s < fstabCols; s++) {
               if (pCurRow[s] != -1) {
                  *pState = pCurRow[s];
                  return j;
               }

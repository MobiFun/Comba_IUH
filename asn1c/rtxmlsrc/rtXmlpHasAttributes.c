...
EXTXMLMETHOD OSBOOL rtXmlpHasAttributes (OSCTXT* pctxt)
{
   ...
   return (OSBOOL) (pXmlReader->mDataMode != OSXMLDM_SIMULATED && 
                    pXmlReader->mbHasAttributes && 
                    pXmlReader->mAttributes.mCount > 0);
}
...
EXTXMLMETHOD void rtXmlpHideAttributes (OSCTXT* pctxt) 
{
   ...
   pXmlReader->mbHasAttributes = FALSE;
}

EXTXMLMETHOD OSBOOL rtXmlpNeedDecodeAttributes (OSCTXT* pctxt) 
{
   return XMLPREADER(pctxt)->mbHasAttributes;
}

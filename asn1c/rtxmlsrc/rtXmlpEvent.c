...
{
   rtXmlRdMarkLastEventActive (XMLPREADER(pctxt));
#ifndef _COMPACT
...
EXTXMLMETHOD OSBOOL rtXmlpIsLastEventDone (OSCTXT* pctxt) {
   OSXMLEvent event = rtXmlRdGetLastEvent (XMLPREADER(pctxt));
   return (OSBOOL)((0 != (event.mId & OSXMLEVT_USED_FLAG)) ?  TRUE : FALSE);
}

...
XML_Parse(XML_Parser parser, const char *s, int len, int isFinal)
{
  ...
  if (len == 0) {
    ...
    */
    ...
    int nLeftOver;
    /* !AB 06/18/2004 
     * XML_Error has been replaced by XML_Status. It is not a big 
     * problem for pure-C, but for C++ compilers (like HP aC++) it 
     * is an error. */
...
{
/*!AB #ifdef XML_CONTEXT_BYTES*/
  if (eventPtr && buffer) {
    *offset = eventPtr - buffer;
    *size   = bufferEnd - buffer;
    return buffer;
  }
/*!AB #endif*/ /* defined XML_CONTEXT_BYTES */
  return (char *) 0;
}

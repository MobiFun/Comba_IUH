...
(((--(pctxt)->buffer.bitOffset < 0) && \
(++(pctxt)->buffer.byteIndex, (pctxt)->buffer.bitOffset = 7)), \
((((pctxt)->buffer.bitOffset < 7) || \
((pctxt)->buffer.byteIndex < (pctxt)->buffer.size)) ? ((*(pvalue) = \
((((pctxt)->buffer.data[(pctxt)->buffer.byteIndex]) >> \
(pctxt)->buffer.bitOffset) & 1)), 0) : RTERR_ENDOFBUF))
...
(OSCTXT* pctxt, OSOCTET** object_p2, OSUINT32* pnumocts);
//(OSCTXT* pctxt, const OSOCTET** object_p2, OSUINT32* pnumocts); Carlo modifica 10/02/2012 per supportare gcc44

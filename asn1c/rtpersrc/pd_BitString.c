...
EXTPERMETHOD int pd_BitString (OSCTXT* pctxt, OSUINT32* numbits_p, 
      ...
      if (bitcnt > 0) {
         *numbits_p += bitcnt;
         ...
         if (pctxt->buffer.aligned) {
            ...
            stat = pu_BitAndOctetStringAlignmentTest (pSizeList, bitcnt, 
                                                      TRUE, &doAlign);
         ...
         stat = pd_octets (pctxt, &buffer[octidx], bufsiz - octidx, bitcnt);
         if (stat != 0) return LOG_RTERR (pctxt, stat);
      }

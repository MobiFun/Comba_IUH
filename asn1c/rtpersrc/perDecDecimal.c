...
int pd_Decimal (OSCTXT *pctxt, const char** object_p) 
{
   int stat;
   OSUINT32 length;
   OSUINT32 flags, tm;
   ...
   if (length == 0) { /* zero */
      ...
      *object_p = p;
      ...
      if ((flags & 0xC0) == 0) { /* base 10 */
         ...
         if (tm >= 1 && tm <= 3) {
            OSUINT32 i;
            char* p = (char*) rtxMemAlloc (pctxt, length);
            if (p == 0) return LOG_RTERRNEW (pctxt, RTERR_NOMEM);
            ...
            *object_p = p;
            ...
            if ((flags & 0x3F) == 3)  { /* convert str to NR1 or NR2 form */
               *object_p = rtxNR3toDecimal (pctxt, *object_p);
               if (*object_p == 0) return LOG_RTERR (pctxt, RTERR_NOMEM);
            }
         }
         else

...
EXTPERMETHOD int pd_Real (OSCTXT* pctxt, OSREAL* pvalue)
{
   ...
   if (firstOctet & REAL_BINARY) {
      ...
      switch(firstOctet & REAL_BASE_MASK) {
         ...
         case (REAL_BASE_8):  base = 8;  break;
         case (REAL_BASE_16): base = 16; break;
         default: return LOG_RTERR (pctxt, RTERR_INVREAL);
      }
      ...
      *pvalue = mantissa * pow ((double)base, (double)exponent);
      ...
      /* this trick is necessary to getting know what is the symbol currently used
       * for decimal mark in current locale. For example, some europeans used ','
       * but americans used '.'.
       */
      sprintf(sbuf, "%1.1f", 0.0);
      mark = sbuf[1];
      if (len < sizeof (sbuf))
         buf = (char*) sbuf;
      else {

...
int berEncStrmObjId (OSCTXT* pctxt, const ASN1OBJID *object_p, 
                     ASN1TagType tagging)
{
   register unsigned i;
   int nbytes;
   ...
   nbytes = rtxGetIdentByteCount (object_p->subid[0] * 40 + object_p->subid[1]);
   for (i = 2; i < object_p->numids; i++) {
      nbytes += rtxGetIdentByteCount (object_p->subid[i]);
   }
   ...
   for (i = 2; i < object_p->numids; i++) {
      stat = berEncStrmIdentifier (pctxt, object_p->subid[i]);
      if (stat != 0) return LOG_RTERR (pctxt, stat);
   }

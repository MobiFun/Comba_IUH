 ...
 */
int xd_bcdstr (ASN1CTXT* pctxt, const char** ppvalue, 
...
{ 
   ...
   if (tagging == ASN1EXPL) {
      if ((stat = xd_match1 (pctxt, ASN_ID_OCTSTR, &length)) < 0)
         ...
         return (stat == ASN_E_IDNOTFOU) ? stat : LOG_ASN1ERR (pctxt, stat);
   }

   if (((pctxt->flags & ASN1CONSTAG) != 0) || (length == ASN_K_INDEFLEN)) {
      return LOG_ASN1ERR (pctxt, ASN_E_NOTSUPP);
   }

   ll = (length * 2) + 1;

   pstr = (char*) rtMemAlloc (pctxt, ll);
   if (0 == pstr) return LOG_ASN1ERR (pctxt, ASN_E_NOMEM);

   *ppvalue = rtBCDToString (length, ASN1BUFPTR(pctxt), pstr, ll);

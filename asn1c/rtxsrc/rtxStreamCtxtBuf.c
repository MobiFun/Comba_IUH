...
(OSRTSTREAM* pStream, const OSOCTET* pdata, size_t numocts)
{
   ...
   if (pctxt->buffer.size > numocts) {
      OSCRTLMEMCPY ((void*)OSRTBUFPTR(pctxt), pdata, numocts);
...
static long ctxtbufRead (OSRTSTREAM* pStream, OSOCTET* pdata, size_t size)
{
   ...
   if (pbuf->byteIndex > 0) {
      ...
      if (nbytesToMove > 0) {
         if (0 != pdata) {
            ...
            OSCRTLMEMCPY (pdata, pbuf->data + pbuf->byteIndex, nbytesToMove);
            ...
            RTHEXDUMP (pctxt, pdata, nbytesToMove);
         }
         else {
            OSCRTLMEMMOVE 
               (pbuf->data, pbuf->data + pbuf->byteIndex, nbytesToMove);
   ...
   if (0 != pdata) {
      ...
      if (nbytesToRead >= (pStream->bufsize - startIndex)) {
         ...
         nbytesToRead = size - startIndex;
         OSCRTLMEMCPY 
            (pdata + startIndex, pbuf->data + pbuf->byteIndex, nbytesToRead);
         ...
         RTDIAG3 (pctxt, "Copied %d bytes from context buffer to buffer %x:\n",
                  nbytesToRead, pdata);
         RTHEXDUMP (pctxt, pdata, pbuf->byteIndex);
      }
   }
...
int rtxStreamCtxtBufCreate (OSCTXT* pctxt)
{
   ...
   if (pctxt->buffer.data == 0) {
       ...
       * when rtMemFree or rtMemReset is called. */

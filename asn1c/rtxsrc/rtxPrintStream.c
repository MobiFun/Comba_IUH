...
EXTRTMETHOD int rtxSetGlobalPrintStream (rtxPrintCallback myCallback, 
                             void * pStrmInfo)
{
   if (myCallback == 0)
...
EXTRTMETHOD int rtxSetPrintStream (OSCTXT* pctxt, rtxPrintCallback myCallback,  
                       void * pStrmInfo)
{
   if (myCallback == 0)
      return LOG_RTERR (pctxt, RTERR_INVPARAM);
   else {
      pctxt->pPrintStrm = rtxMemAllocType (pctxt, OSRTPrintStream);
      if (pctxt->pPrintStrm == NULL) return LOG_RTERR (pctxt, RTERR_NOMEM);

      rtxMemHeapMarkSaved (&pctxt->pMemHeap, pctxt->pPrintStrm, TRUE);
...
EXTRTMETHOD int rtxDiagToStream (OSCTXT* pctxt, const char* fmtspec, va_list arglist)
{
   if (pctxt != 0 && pctxt->pPrintStrm != 0) {
      ((rtxPrintCallback)pctxt->pPrintStrm->pfPrintFunc)
         (pctxt->pPrintStrm->pPrntStrmInfo, fmtspec, arglist);
...
EXTRTMETHOD int rtxPrintStreamRelease (OSCTXT* pctxt)
{
   ...
   if (pctxt->pPrintStrm != 0) {
      rtxMemFreeType (pctxt, pctxt->pPrintStrm);
      pctxt->pPrintStrm = 0;
   }

   ...
   mSocket (socket)
{
   ...
   mStatus = mInitStatus = 
      rtxStreamSocketAttach (mpContext->getPtr (), mSocket.getSocket (), 
                             OSRTSTRMF_INPUT);
...
EXTRTMETHOD OSRTSocketInputStream::OSRTSocketInputStream (OSRTSOCKET socket, OSBOOL ownership):
   mSocket (socket, ownership)
{
   if (!isInitialized ()) return;

   mStatus = mInitStatus = rtxStreamSocketAttach 
      (mpContext->getPtr (), mSocket.getSocket (), OSRTSTRMF_INPUT);
   if (mInitStatus != 0) LOG_RTERR (mpContext->getPtr (), mInitStatus);
}

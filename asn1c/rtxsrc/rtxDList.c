...
(OSCTXT* pctxt, OSRTDList* pList, OSUINT32 index, void* pData)
{
   OSRTDListNode* pListNode;
   ...
   if (index > pList->count) {
      ...
      if (0 != pListNode) {
         pListNode->data = pData;
         rtxDListInsertNode (pList, index, pListNode);
      }
   }
...
(OSRTDList* pList, OSUINT32 index, OSRTDListNode* pListNode)
{
   if (0 != pListNode) {
      if (index == 0) { /* index 0 - first element */
      ...
      else if (index == pList->count) { /* last index (tail case) */
         pListNode->next = (OSRTDListNode*) 0;
         if (0 != pList->tail) {
            pList->tail->next = pListNode;
            pListNode->prev = pList->tail;
         ...
         for (i = 1, curNode = pList->head; i < index; i++) {
...
OSRTDListNode* rtxDListFindByIndex (const OSRTDList* pList, OSUINT32 index) 
{
   ...
   if (index < (OSUINT32) (pList->count / 2)) {
      for(i = 0, curNode = pList->head; i < index; i++) {
         curNode = curNode->next;
      }
   }
   else {
      for(i = pList->count - 1, curNode = pList->tail; i > index; i--) {
...
(struct OSCTXT* pctxt, OSRTDList* pList, void** ppArray, 
 OSUINT32* pElemCount, size_t elemSize)
{
   OSRTDListNode* pNode;
   size_t i = 0;
   OSUINT32 arrayCount = (0 == pElemCount) ? 0 : *pElemCount;
   ...
   for (pNode = pList->head; pNode != 0; pNode = pNode->next, i += elemSize) {
      OSCRTLMEMCPY (((OSOCTET*)*ppArray) + i, pNode->data, elemSize);
   }
   ...
   if (slen > 0) {
      *ppstr = sp = (OSUTF8CHAR*) rtxMemAlloc (pctxt, slen);
      ...
      for (pNode = pList->head; pNode != 0; pNode = pNode->next) {
         ...
         OSCRTLMEMCPY (sp, pNode->data, slen);
         sp += slen;
...
(OSCTXT* pctxt, OSRTDList* pList, void* pArray, 
 OSUINT32 numElements, size_t elemSize)
{
   OSUINT32 i;
   void* pData;
 ...
 OSUINT32 numElements, size_t elemSize)
{
   ...
   for (i = 0; i < numElements; i++) {
      ...
      OSCRTLMEMCPY (pData, ((const OSOCTET*)pArray) + (i * elemSize), elemSize);
...
EXTRTMETHOD void rtxDListBufInit (OSRTDListBuf* pBuf, OSUINT32 segSz, 
                                  void** ppdata, size_t elemSz)
{
   if (pBuf) {
      pBuf->n = 0;
      pBuf->nMax = 0;
   ...
   if (pBuf) {
      if (pBuf->tmplist.count) {
         ...
         *(pBuf->dataArray) = rtxMemAlloc (pctxt, pBuf->nAll * pBuf->elemSize);
         ...
         while (pnode) {
            OSUINT32 rest = pBuf->nAll - pBuf->n;
            if (rest > pBuf->nMax) rest = pBuf->nMax;

            OSCRTLMEMCPY ((char*)*(pBuf->dataArray) + pBuf->n * pBuf->elemSize, 
               pnode->data, pBuf->elemSize * rest);

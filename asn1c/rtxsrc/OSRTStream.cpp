...
{
   mStatus = mInitStatus = mpContext->getStatus ();
   if (mInitStatus != 0) return;

   mStatus = mInitStatus = rtxStreamInit (mpContext->getPtr ());
   if (mInitStatus < 0) return;
}

EXTRTMETHOD OSRTStream::OSRTStream (OSRTContext* pContext, OSBOOL attachStream) :
   OSRTCtxtHolder (pContext), mbAttached (attachStream)
{
...
EXTRTMETHOD int OSRTStream::close ()
{
   ...
   mStatus = rtxStreamClose (mpContext->getPtr ());
...
EXTRTMETHOD int OSRTStream::flush ()
{
   ...
   mStatus = rtxStreamFlush (mpContext->getPtr ());
   if (mStatus < 0) return LOG_RTERR (mpContext->getPtr (), mStatus);
   return 0;
}
...
{ 
   return rtxStreamIsOpened (mpContext->getPtr ()); 
}

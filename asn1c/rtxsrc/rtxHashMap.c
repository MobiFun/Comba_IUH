...
static int hashMapExpand (OSCTXT* pctxt, HASHMAPTYPENAME* pHashMap)
{
   ...
   for (i = 0; i < pHashMap->tableLength; i++) {
      while (0 != (pEntry = pHashMap->table[i])) {
         ...
         index = indexFor (newsize, pEntry->hashCode);
         pEntry->next = ppNewTable[index];
         ppNewTable[index] = pEntry;
      }
   }
   rtxMemFreePtr (pctxt, pHashMap->table);
 ...
 HASHMAPVALUETYPE value)
{
   ...
   index = indexFor (pHashMap->tableLength, pEntry->hashCode);
   pEntry->key = key;
   pEntry->value = value;
   pEntry->next = pHashMap->table[index];
   pHashMap->table[index] = pEntry;
...
(OSCTXT* pctxt, HASHMAPTYPENAME* pHashMap, HASHMAPKEYTYPE key, 
 HASHMAPVALUETYPE value)
{
   OSUINT32 hashCode, index;
   HASHMAPENTRYTYPE* pEntry;

   hashCode = computeHashCode (pHashMap, key);
   index = indexFor (pHashMap->tableLength, hashCode);

   /* Search chain for key */

   pEntry = pHashMap->table[index];
   while (0 != pEntry) {
...
{
   OSUINT32 hashvalue, index;
   HASHMAPENTRYTYPE* pEntry;

   hashvalue = computeHashCode (pHashMap, key);
   index = indexFor (pHashMap->tableLength, hashvalue);
   pEntry = pHashMap->table[index];
   while (0 != pEntry) {
      /* Check hash value to short circuit heavier comparison */

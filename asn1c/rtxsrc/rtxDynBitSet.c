...
(OSCTXT* pctxt, const OSRTDynBitSet* pSrcBitSet, OSRTDynBitSet* pDestBitSet)
{
   ...
   OSCRTLMEMCPY (pDestBitSet->data, pSrcBitSet->data, pSrcBitSet->segsize);
   return 0;
}
...
{
   if (index > OSUINT16_MAX)
      return LOG_RTERR (pctxt, RTERR_OUTOFBND);
   
   if (index >= (OSUINT32)(pbitset->nbytes * 8)) {
      /* expand buffer */
      size_t newSize = (index / (pbitset->segsize * 8) + 1) * pbitset->segsize;
   ...
   rtxSetBit (pbitset->data, pbitset->nbytes * 8, index);
   if (index >= pbitset->maxbit) pbitset->maxbit = (OSUINT16)(index + 1);
   return 0;
}

EXTRTMETHOD int rtxDynBitSetClearBit (OSRTDynBitSet* pbitset, OSUINT32 index)
{
   if (index < (OSUINT32)(pbitset->nbytes * 8)) {
      rtxClearBit (pbitset->data, pbitset->nbytes * 8, index);
      if ((index + 1) == pbitset->maxbit) {
         /* compute the new maxbit value */
         OSOCTET* p = pbitset->data + index / 8;
         OSOCTET* pEnd = pbitset->data;
         OSUINT32 maxbit = index / 8 * 8 + 7;
...
{
   return (OSBOOL)
      ((index >= pbitset->maxbit) ? 
       FALSE : rtxTestBit (pbitset->data, pbitset->maxbit, index));
}
...
{
   return (value) ? 
      rtxDynBitSetSetBit (pctxt, pbitset, index) :
      rtxDynBitSetClearBit (pbitset, index);
}

EXTRTMETHOD int rtxDynBitSetInsertBit 
(OSCTXT* pctxt, OSRTDynBitSet* pbitset, OSUINT32 index, OSBOOL value)
{
   if (index < pbitset->maxbit) {
      OSOCTET* p = pbitset->data + index / 8;
      ...
      /* shift part of byte */
      c = *p;
      mask = (OSOCTET) (0xFF >> (index % 8));
      *p = (OSOCTET) ((c & ~mask) | ((c & mask) >> 1));
      if (value) *p |= 0x80 >> (index % 8);
      ...
      int stat = rtxDynBitSetSetBitToValue (pctxt, pbitset, index, value);
      if (0 != stat) return LOG_RTERR (pctxt, stat);
   }
